...
INVOICE_ACCESS_SECRET = os.environ.get("INVOICE_ACCESS_SECRET", "invoice-access-secret")
INVOICE_SYSTEM_URL = os.environ.get("INVOICE_SYSTEM_URL", "http://127.0.0.1:5001")
...
def _build_invoice_access_signature(username, role, issued_at):
    payload = f"{username}|{role}|{issued_at}"
    return hmac.new(
        INVOICE_ACCESS_SECRET.encode("utf-8"),
        payload.encode("utf-8"),
        hashlib.sha256,
    ).hexdigest()


def _build_invoice_url(username, role, lead_id=None):
    issued_at = int(time.time())
    signature = _build_invoice_access_signature(username, role, issued_at)
    query = {
        "user": username,
        "role": role,
        "ts": issued_at,
    ...
    if lead_id is not None:
        return f"{INVOICE_SYSTEM_URL}/invoices/create-ui?{urlencode({**query, 'lead_id': lead_id})}"
    return f"{INVOICE_SYSTEM_URL}/?{urlencode(query)}"
...
def payments():
    ...
    return render_template(
        ...
        invoice_system_url=INVOICE_SYSTEM_URL,
        invoice_create_base_url=_build_invoice_url(current_user.username or "accounts", "accounts"),
    )


@accounts_bp.route("/invoice-portal")
@login_required
def invoice_portal():
    if not require_accounts():
        return redirect(url_for("index"))

    username = current_user.username or "accounts"
    role = "accounts"
    lead_id = request.args.get("lead_id", type=int)
    return redirect(_build_invoice_url(username, role, lead_id))
