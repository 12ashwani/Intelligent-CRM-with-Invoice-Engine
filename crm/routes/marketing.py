...
def create():
    ...
    try:
        ...
        if lead_status == "Converted":
            operation_executive = request.form.get("operation_executive")
            if operation_executive:
                assign_to_operations(new_lead_id, int(operation_executive))
...
# 🔄 UPDATE LEAD STATUS
# ================================
@marketing_bp.route("/update-status/<int:lead_id>", methods=["POST"])
@login_required
def update_lead_status(lead_id):
    ...
    try:
        ...
        cur.execute(
            ...
            SELECT id
            FROM leads
            WHERE id=%s AND marketing_executive=%s
            """,
            (lead_id, current_user.employee_id)
        )
