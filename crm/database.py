...
def create_tables():
    ...
    # ----- INVOICE MODULE (existing tables from earlier CRM version) -----
    # These tables are kept for backwards compatibility and will be extended with new fields for GST compliance.
    ...
    # ----- NEW INVOICE SCHEMA (GST invoices with HSN, tax tables, etc.) -----
    # Company table (separate from company_settings; used for multi‑company billing)
    cur.execute("""
    ...
    # Extend customers table with shipping details (if not exists)
    for col, col_def in [
    ...
    # Extend invoices table with new GST invoice fields
    invoice_new_columns = [
        ("company_id", "INT", "FOREIGN KEY (company_id) REFERENCES company(id) ON DELETE SET NULL"),
        ("date", "DATE"),
        ("due_date", "DATE"),
        ...
        ("total_in_words", "TEXT"),
        ("tax_in_words", "TEXT"),
        ("cgst_rate", "DECIMAL(5,2) DEFAULT 0"),      # to store rates at invoice level
        ("sgst_rate", "DECIMAL(5,2) DEFAULT 0"),
        ("igst_rate", "DECIMAL(5,2) DEFAULT 0"),
    ]
    for col, col_type, *fk in invoice_new_columns:
        try:
            if fk:
                cur.execute(f"ALTER TABLE invoices ADD COLUMN {col} {col_type}")
                # add foreign key separately (MySQL requires constraint name)
    ...
    # Extend invoice_items with HSN, description, unit, line_total
    item_new_columns = [
        ("description", "TEXT"),
        ("hsn", "VARCHAR(50)"),
        ("unit", "VARCHAR(50)"),
    ...
    for col, col_type in item_new_columns:
        try:
            cur.execute(f"ALTER TABLE invoice_items ADD COLUMN {col} {col_type}")
        except:
            pass

    # Create invoice_tax table (HSN‑wise tax breakdown)
    cur.execute("""
    ...
    # Indexes for performance (MySQL doesn't support CREATE INDEX IF NOT EXISTS)
...
def create_default_admin():
    ...
    admin_user: Optional[Dict[str, Any]] = cur.fetchone()
...
def generate_invoice_number(invoice_type: str) -> str:
    ...
    Example: PI/2025/001, TAX/2025/001
    """
...
def create_invoice(data: Dict) -> int:
    ...
    Expected data structure:
    {
        ...
        'items': [
            {'name': str, 'description': str, 'hsn': str, 'qty': float,
             'unit': str, 'price': float, 'line_total': float}
        ],
        'tax_lines': [
            {'hsn': str, 'taxable_value': float, 'rate': float,
             'cgst_rate': float, 'cgst_amount': float,
             'sgst_rate': float, 'sgst_amount': float,
             'igst_rate': float, 'igst_amount': float, 'amount': float}
        ]
    }
    """
    conn = get_db_connection()
